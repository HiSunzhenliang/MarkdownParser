...
本项目将针对 Markdown 的一些最常用语法，手动实现一个 Markdown 解析器。
...
为了测试 Markdown 的解析结果，把解析结果直接转换到 HTML 进行展示，所以，这里已经准备好了 CSS 文件。
...
要解析 Markdown 文件，要先在程序中定义好相关的词法信息、语法规则。

这里如果对编译原理有所了解，就能够立刻明白对 Markdown 的解析，其实就是编译过程中的词法分析和语法分析阶段，而最后的到 HTML 的转换，实际上就是『目标代码的生成』这个阶段。
...
根据前面的代码，可以将 `MarkdownTransform` 类设计成如下的形式：
...
构造函数接口给定的是需要解析的文件名，那么自然就需要从文件流中读取。
